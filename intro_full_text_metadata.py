    ...
    try:
        response = await client.chat.completions.create(
            ...
            messages=[
                ...
                {
                    "role": "user",
                    "content": f"Consise the context for further analysis to find the informative insights from the context: {context}",
                },
            ],
        )
        return (
    ...
    if len(contexts) == 1:
        ...
        async def process_texts_async(contexts, logger):
            async with AsyncOpenAI(api_key=os.getenv("OPENAI_SECRET_KEY")) as client:
                tasks = [
                    process_text_async(client, context, logger) for context in contexts
                ]
...
def process_df(df: DataFrame, logger: Logger, assistant: str, thread: str):
    """Aggredate the data into list elements"""
    contexts = aggregate_into_few(df=df, logger=logger)

    message = client.beta.threads.messages.create(
        thread_id=thread, role="user", content=get_prompt(contexts[0])
    )

    run = client.beta.threads.runs.create(thread_id=thread, assistant_id=assistant)

    while True:
        run = client.beta.threads.runs.retrieve(thread_id=thread, run_id=run.id)
        if run.status == "completed":
            messages = client.beta.threads.messages.list(thread_id=thread, limit=1)
            message_content = messages.data[0].content[0].text
            # new_df = json.loads(message_content.value)
            new_df = message_content.value

            match = re.search(r"```json(.*?)```", new_df, re.DOTALL)
            try:
                if match:
...
def extract_record_text(
    ...
    try:
        ...
        if len(fulltext) > 0:
            ...
            df = process_df(df=df, logger=logger, assistant=assistant, thread=thread)
    ...
    for record in result:
        ...
        try:
            ...
            if metadata is not None:
                ...
                response = {"id": record.get("id"), "metadata": metadata}
                logger.info(
                    f"Content id : {response['id']}, Meta keywords : {response['metadata']['meta_keywords']}, Meta description : {response['metadata']['meta_description']}, Tags : {response['metadata']['meta_tags']}"
                )
                ...
                if commit:
                    succeed = do_update(
                        connection=connection,
                        alias=record["alias"],
                        metadata=response["metadata"]["meta_keywords"],
                        description=response["metadata"]["meta_description"],
                        ...
                        tags=response["metadata"]["meta_tags"],
...
def main(
    ...
    assistant = client.beta.assistants.create(
        ...
        instructions="""Analyze the provided context from a website targeting Linux and open-source security professionals to generate SEO-optimized meta tags, meta keywords, and a concise meta description. As an SEO specialist, carefully examine the content to identify the most relevant and unique keywords that reflect the core topics and themes. Ensure the meta keywords list includes the top 5 to 8 distinct keywords, each surrounded by quotes and highly relevant to the context of Linux and open-source security. Additionally, generate meta tags that align closely with the identified keywords, ensuring they are unique and directly pertinent to the content. Craft a meta description that is clear, concise, and no longer than 20 words, effectively summarizing the main value or insight provided by the content. This description should capture the essence of the website's focus on Linux and open-source security for professionals. Provide the output in a structured JSON format, including fields for meta_keywords, meta_tags, and meta_description.""",
    )
    thread = client.beta.threads.create()

    while True:
        try:
            result = get_limit_rows(
                connection=connection,
                limit=limit,
                current_id=current_id,
                id=id,
                max_records=max_records_runs,
            )
            if not result:
            ...
            counter = process_records(
                ...
                max_tokens=max_tokens,
                json_file=json_file,
                store_state_file=store_state_file,
                commit=commit,
                connection=connection,
                base_url=base_url,
